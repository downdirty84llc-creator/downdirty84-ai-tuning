...
stripeRouter.post("/webhook", async (req, res) => {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      `, [userId, event.id, session.id, email, amount, currency, jsonb(items)]);
        ...
        if (amount == 7900 || desc.includes("priority log"))
            serviceType = "PRIORITY_LOG_REVIEW";
        if (amount == 24900 || (desc.includes("stage 1") && desc.includes("na")))
            serviceType = "STAGE1_NA";
        ...
        if (!serviceType) {
            ...
            if (amounts.includes(24900))
                serviceType = "STAGE1_NA";
            else if (amounts.includes(39900))
                serviceType = "STAGE1_BOOST";
            else if (amounts.includes(7900))
                serviceType = "PRIORITY_LOG_REVIEW";
            else if (amounts.includes(3900))
                serviceType = "LOG_REVIEW";
        }
        if (!serviceType)
            return res.json({ received: true });
