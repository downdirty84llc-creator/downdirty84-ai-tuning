...
import { createUpload, listUploadsForJob, attachUploadsToJob } from "./uploads.repo.js";
...
uploadsRouter.post("/", requireAuth, upload.single("file"), async (req, res) => {
    ...
    if (!req.file)
        return badRequest(res, "Missing file.");
...
uploadsRouter.post("/jobs/:jobId/attach", requireAuth, async (req, res) => {
    ...
    if (!Array.isArray(uploadIds) || uploadIds.length === 0)
        return badRequest(res, "uploadIds required.");
