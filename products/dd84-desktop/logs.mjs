...
export async function logReport(raw,name,tuneSha256='',associationNote=''){
  ...
  return {format:'DD84_LOG_INSPECTION_V1',status:'EVIDENCE_ONLY_NOT_RELEASED',writeStrategy:'SIMULATION_ONLY',source:{name,textSha256:Array.from(new Uint8Array(digest),x=>x.toString(16).padStart(2,'0')).join(''),hashScope:'UTF8_DECODED_TEXT'},calibration:{sha256:tuneSha256.toLowerCase()||null,association:tuneSha256?'OPERATOR_SUPPLIED_NOT_VERIFIED':'UNCONFIRMED',note:associationNote.trim()},...summary,automaticApplication:false,learningReady:false,limitations:['Blank cells remain missing; no interpolation or forward fill is performed.','Row counts are export rows, not a uniform sampling rate.','Ranges are observed values, not validated measurements or recommended settings.','Channel names and units are preserved without mapping them to MAF, VE, lambda or knock inputs.','This inspection does not establish performance improvement or enable correction learning.']};
}
