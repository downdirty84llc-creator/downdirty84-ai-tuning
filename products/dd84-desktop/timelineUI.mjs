...
export function renderTimeline(raw,channelId){
  ...
  try{
    const t=channelTimeline(raw,channelId),unit=t.channel.unit||'unit not supplied';
    description.textContent=t.channel.name+' ['+unit+']: '+t.numericCount+' numeric readings; '+t.missingCount+' missing cells; '+t.textCount+' nonnumeric cells. '+t.bins.length+' time intervals. Each mark shows an interval’s recorded minimum and maximum; blank intervals have no numeric readings. No values are filled or connected.';
    ...
    chart.append(svg);details.textContent=t.bins.map((b,i)=>'['+b.start.toFixed(3)+', '+b.end.toFixed(3)+(i===t.bins.length-1?']':')')+' s: '+(b.numericCount?b.min+' to '+b.max+' '+unit:'no numeric readings')+'; numeric '+b.numericCount+', missing '+b.missingCount+', nonnumeric '+b.textCount).join('\n');
