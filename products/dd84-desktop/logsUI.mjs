...
function showTimeRange(){
  ...
  renderTimeline(raw,$('log-trace-channel').value,{start:start.value.trim()?start.valueAsNumber:NaN,end:end.value.trim()?end.valueAsNumber:NaN});
}
...
for(const id of ['log-trace-start','log-trace-end'])$(id).oninput=()=>{renderTimeline(null,'');$('log-trace-description').textContent='Select Show time range to update the view.';};
